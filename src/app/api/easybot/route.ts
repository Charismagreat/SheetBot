...
2. 🚀 [이지데스크 터널 클라이언트 지식]:
   ...
   - 🏢 [공공 및 기업 인텔리전스 신규 도구 지원]:
     * 국민연금(NPS) 사업장/직원수/추이: egdeskToolsCall('nps', 'nps_lookup', { workplaceName: '회사명' })
     * 나라장터(KONEPS) 계약 실적: egdeskToolsCall('koneps', 'koneps_lookup', { companyName: '회사명' })
     * 나라장터 입찰공고(BidNotice): egdeskToolsCall('bidnotice', 'bidnotice_lookup', { title: '검색어' })
     * 기업 심층 웹 리서치: egdeskToolsCall('company-research', 'companyresearch_run', { domain: '도메인' })
