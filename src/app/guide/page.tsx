...
export default function GuidePage() {
  ...
  const steps = [
    ...
    {
      step: "02",
      title: "AI 시트 2차원 정밀 분석 & 대화형 실행 계획 조율 (HITL)",
      desc: "시트 URL을 입력하면 AI 아키텍트가 시트 전체(모든 탭, 헤더 열, 데이터 샘플)를 실시간 스캔하여 양식 유형(누적 대장형, 고정 양식형 등)과 실행 계획을 브리핑합니다. 최대 5회까지 AI와 자유롭게 대화하며 컬럼 매핑과 로직을 사전 조율할 수 있습니다.",
      tip: "예: 'D열 품번은 없으면 빈칸으로 두고 E열 품명만 필수로 넣어줘' 등 편하게 한국어로 피드백하세요.",
      badge: "대화형 조율",
    },
    {
      step: "03",
      title: "Google Apps Script 원클릭 구글 클라우드 자동 배포",
      desc: "검증된 계획을 바탕으로 '구글 시트에 배포하기'를 누르면 이지데스크 클라우드 터널을 통해 대상 시트에 완성형 Google Apps Script 코드가 버전 관리되며 안전하게 자동 주입됩니다.",
      tip: "구글 시트 상단 메뉴에 '🚀 SheetBot 자동화' 전용 메뉴가 자동으로 등록되어 시트 안에서도 원클릭 실행이 가능합니다.",
      badge: "원클릭 배포",
    },
    {
      step: "04",
      title: "구글 시트 사이드바 AI OCR 문서 분석 & 다중 품목 자동 기록",
      desc: "구글 시트 메뉴 [🚀 SheetBot 자동화]를 열고 PDF나 이미지 발주서·영수증을 올리면, AI가 문서를 실시간 분석하여 시트 컬럼 규격에 1:1로 매핑합니다. 1장에 여러 품목이 있어도 품목별로 1행씩 깔끔하게 분리되어 최신순으로 자동 기록됩니다.",
      tip: "개인 API 키 등록 없이 원클릭 작동하며, 소비된 토큰은 'AI 사용료' 관제 센터에서 투명하게 실시간 조회됩니다.",
      badge: "문서 AI OCR",
    },
    {
      step: "05",
      title: "요구사항 수정 & 스마트 알림(내 폰 문자) 및 실시간 관제",
      desc: "이미 배포된 프로젝트도 [요구사항 수정]을 통해 언제든 AI 시트 재분석과 대화 조율을 거쳐 새 코드로 덮어쓸 수 있습니다. 상단 [스마트 알림]으로 본인 안드로이드 폰을 QR 연동하여 통신비 0원 자동 문자 발송도 가능합니다.",
      tip: "상단 [AI 사용료] 메뉴에서 모든 AI 호출 목적과 토큰 소비 내역을 실시간 감사 로그로 확인할 수 있습니다.",
      badge: "스마트 운영",
    },
  ];
