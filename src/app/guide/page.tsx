...
export default function GuidePage() {
  ...
  const steps = [
    {
      step: "01",
      title: "구글 계정 로그인 & 3대 프로젝트 시작 모드 선택",
      desc: "별도 가입 없이 구글 계정으로 로그인하면 20,000 웰컴 토큰이 즉시 지급됩니다. [새 프로젝트 추가]에서 ① '✨ 새 시트 자동 생성' (시트 없이 아이디어만으로 양식 설계), ② '📁 엑셀 파일 업로드 변환' (.xlsx/.csv 드래그앤드롭 파싱), ③ '🔗 기존 시트 URL' 중 원하는 방식을 자유롭게 선택하세요.",
      tip: "엑셀 파일(.xlsx, .csv)을 끌어다 놓으면 컬럼 헤더와 데이터가 1초 만에 자동 분석되어 미리보기로 뜹니다.",
      badge: "3대 모드 지원",
    },
    {
      step: "02",
    ...
    },
    {
      ...
      desc: "이미 배포된 프로젝트도 [요구사항 수정]을 통해 언제든 AI 시트 재분석과 대화 조율을 거쳐 새 코드로 업데이트할 수 있습니다. [스마트 알림 센터]에서 본인 안드로이드 폰을 QR 연동하여 통신비 0원 자동 문자 발송도 가능합니다.",
