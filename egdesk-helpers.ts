 ...
 * internal knowledge / business identity / company research (MCP), company research docs (cited tool-result blocks), Korean law (MCP), Bizinfo grants (MCP), NPS workplace headcount (MCP), and browser recorder replay / live browse.
...
// ==========================================
// COMPANY RESEARCH DOCS (interactive cited canvases)
// ==========================================

/** List interactive company research docs (one per businessNumber) */
export async function listCompanyResearchDocs() {
  return callCompanyResearchTool('companyresearch_doc_list', {});
}

/** Get company research doc by businessNumber or docId */
export async function getCompanyResearchDoc(options: {
  businessNumber?: string;
  docId?: string;
}) {
  return callCompanyResearchTool('companyresearch_doc_get', options);
}

/** Get or create the doc for a company (unique per businessNumber) */
export async function getOrCreateCompanyResearchDoc(input: {
  businessNumber: string;
  companyName?: string;
  domain?: string;
  title?: string;
}) {
  return callCompanyResearchTool('companyresearch_doc_get_or_create', input);
}

/**
 * Run a company-research tool and append a cited block to the company doc —
 * same as "문서에 넣기" in the Company Research doc editor UI.
 */
export async function insertCompanyResearchDocToolResult(input: {
  businessNumber: string;
  toolName: string;
  ...
  return callCompanyResearchTool('companyresearch_doc_insert_tool_result', input);
}

/** Append already-fetched tool JSON to a company doc without re-running the tool */
export async function insertCompanyResearchDocResult(input: {
  businessNumber: string;
  toolName: string;
  rawResult: unknown;
  companyName?: string;
  ...
  return callCompanyResearchTool('companyresearch_doc_insert_result', input);
}

/** Fixed proposal deck layouts and hard content limits (no LLM). */
export async function getProposalLayoutCatalog() {
  return callCompanyResearchTool('companyresearch_proposal_layout_catalog', {});
}

/** Validate structured proposal slide JSON → placeholders + slides[] for export. */
export async function parseProposalSlideContent(content: unknown) {
  return callCompanyResearchTool('companyresearch_proposal_parse_content', { content });
}

/** LLM: excerpt → structured proposal content (same as Doc Editor slide-from-text). */
export async function structureCompanyResearchDocSlideFromText(text: string) {
  return callCompanyResearchTool('companyresearch_doc_slide_from_text', { text });
}

/** Append slides[] to a company research doc deck. */
export async function appendCompanyResearchDocSlides(input: {
  businessNumber?: string;
  docId?: string;
  slides: unknown[];
  companyName?: string;
  ...
  return callCompanyResearchTool('companyresearch_doc_append_slides', input);
}
 ...
 * Pass preferOAuth: true (default) to create in the signed-in user's Drive when a service account is also configured.
 */
export async function createGoogleSlides(
  ...
  const preferOAuth = options.preferOAuth ?? true;
  return callSlidesTool('slides_create', { title, preferOAuth }, options);
}

export async function getGoogleSlides(
  presentationId: string,
  options: WorkspaceVisitorCallOptions = {},
) {
  const preferOAuth = options.preferOAuth ?? true;
  return callSlidesTool('slides_get', { presentationId, preferOAuth }, options);
}

export async function getGoogleSlidesText(
  presentationId: string,
  options: WorkspaceVisitorCallOptions = {},
) {
  const preferOAuth = options.preferOAuth ?? true;
  return callSlidesTool('slides_get_text', { presentationId, preferOAuth }, options);
}

export async function addGoogleSlide(
  presentationId: string,
  options: { text?: string; insertionIndex?: number; preferOAuth?: boolean } = {},
  callOptions: WorkspaceVisitorCallOptions = {},
) {
  const preferOAuth = options.preferOAuth ?? callOptions.preferOAuth ?? true;
  return callSlidesTool('slides_add_slide', { presentationId, ...options, preferOAuth }, callOptions);
}
...
export async function replaceGoogleSlidesText(
  ...
  const preferOAuth = options.preferOAuth ?? true;
  return callSlidesTool(
    'slides_replace_text',
    { presentationId, findText, replaceText, matchCase, preferOAuth },
    options,
  );
}
...
export async function insertGoogleSlidesImage(
  ...
  const preferOAuth = options.preferOAuth ?? callOptions.preferOAuth ?? true;
  return callSlidesTool('slides_insert_image', { ...options, preferOAuth }, callOptions);
}

export async function getGoogleSlidesProposalLayoutCatalog() {
  return callSlidesTool('slides_proposal_layout_catalog', {});
}

/** Append a slide from structured proposal content to a Google Slides deck. */
export async function fillGoogleSlideFromProposalContent(
  presentationId: string,
  content: unknown,
  options: WorkspaceVisitorCallOptions = {},
) {
  const preferOAuth = options.preferOAuth ?? true;
  return callSlidesTool(
    'slides_fill_from_content',
    { presentationId, content, preferOAuth },
    options,
  );
}
 ...
 * snapshot links, inbound/outbound rules, background listener).
 *
...
export async function getPhoneMessages(options: {
  deviceId: string;
  phoneNumber?: string;
  convKey?: string;
  title?: string;
  ...
  return callPhoneTool('phone_get_messages', options);
}
...
export async function enqueuePhoneSms(options: {
  deviceId: string;
  phoneNumber: string;
  message: string;
  snapshotId?: string;
  ...
  return callPhoneTool('phone_enqueue', options);
}
...
export async function listPhoneSnapshotLinks() {
  return callPhoneTool('phone_list_snapshot_links', {});
}

export async function listPhoneSnapshotsForDevice(deviceId: string) {
  return callPhoneTool('phone_list_snapshots_for_device', { deviceId });
}
...
export async function setPhoneConsentAction(options: {
  snapshotId: string;
  phone: string;
  action: 'opt_in' | 'opt_out';
}) {
  return callPhoneTool('phone_consent_action_set', options);
}

export async function createPhoneMessageRule(options: {
  deviceId: string;
  enabled?: boolean;
  matchPhone?: string;
  matchBodyContains?: string;
  matchBodyRegex?: string;
  matchDirection?: 'incoming' | 'outgoing' | 'any';
  actionType: 'script' | 'send_sms' | 'webhook';
  actionPayload: Record<string, any>;
  cooldownMs?: number;
}) {
  return callPhoneTool('phone_rule_create', options);
}

export async function updatePhoneMessageRule(options: {
  ruleId: string;
  enabled?: boolean;
  matchPhone?: string;
  matchBodyContains?: string;
  ...
  return callPhoneTool('phone_rule_update', options);
}

export async function deletePhoneMessageRule(ruleId: string) {
  return callPhoneTool('phone_rule_delete', { ruleId });
}

export async function listPhoneMessageRules(deviceId?: string) {
  return callPhoneTool('phone_rule_list', deviceId ? { deviceId } : {});
}

export async function getPhoneListenerStatus(deviceId?: string) {
  return callPhoneTool('phone_listener_status', deviceId ? { deviceId } : {});
}

export async function testPhoneListener(options: {
  deviceId: string;
  body: string;
  phone?: string;
  ...
  return callPhoneTool('phone_listener_test', options);
}
