...
8. **빈 시트(NEW_SHEET) 초기화 시 기본 탭(`Sheet1`) 무손실 단일화(Rename) 원칙**:
   - 구글 스프레드시트가 새로 생성될 때 구글 시스템에 의해 기본 빈 탭(`Sheet1` 또는 `시트1`)이 무조건 1개 자동 생성됩니다.
   - 따라서 초기 레이아웃(`setupInitialSheetLayout`)을 구성할 때, `insertSheet`로 새 탭을 추가하여 탭을 2개로 쪼개지 말고, 반드시 기존 기본 탭이 1개뿐인 경우 `sheets[0].setName(TARGET_SHEET_NAME)`으로 이름을 변경하여 **단 1개의 메인 탭만 유지**해야 합니다.
9. **에이전트 브릿지 '빈 시트 래핑' 시 임의 시트 차용 절대 금지 (Strict No-Arbitrary Mapping)**:
   - 브릿지 요청 수신 시 `spreadsheetId`가 비어있거나(`null`) 신규 시트 생성이 실패했을 때, 사용자의 구글 드라이브에 있는 과거 기존 시트의 ID를 임의로 탐색하여 연결하거나 덮어씌워 배포해서는 안 됩니다.
   - 시트 ID가 유효하지 않거나 자동 생성이 불가능한 경우, 즉시 작업을 멈추고 사용자에게 신규 시트 URL을 요청해야 합니다.
10. **프로젝트명 및 구글 시트 파일명 표준 명명 원칙 (Dual Naming Policy)**:
   - **[빈 시트로 새로 생성(`NEW_SHEET`)]**:
     - 구글 스프레드시트 파일명과 시트봇 대시보드 프로젝트명 모두 **`[SheetBot] {업무명}`**으로 통일 적용합니다 (예: `[SheetBot] 스마트 명함 관리 대장`).
...
## 스마트폰 SMS 문자 발송 및 시트봇 에이전트 연동 표준 준수 원칙

1. **에이전트 역할 및 명칭 분리 절대 준수 원칙 (시트봇 에이전트 M vs 시트봇 에이전트)**:
   - **관리자용 (`시트봇 에이전트 M` / `SheetBot Agent M`)**: 운영자 전용 무통장 입금 감지 및 전역 토큰 지갑 자동 충전 시스템 (`/dashboard/deposit-agent`).
   - **이용자용 (`시트봇 에이전트` / `SheetBot Agent`)**: 일반 회원이 자신의 구글 스프레드시트와 스마트폰을 1:1로 결합하여 0원 고객 알림 문자를 발송하고 수신 문자를 시트에 자동 기록하는 양방향 에이전트 (`/dashboard/notifications`).
...
3. **발송 전 기기 실시간 점검(`checkActiveSmsDevice`) 및 계정 1:1 매칭 필수**:
   - My DB(`sheetbot_user_devices`)의 터널 응답(JSON)을 안전하게 언래핑하여 로그인된 세션 이메일(`SHEETBOT_USER_EMAIL`)에 매핑된 활성 기기(`SheetBot Agent`, status === 'CONNECTED')를 1순위로 탐색해야 합니다.
4. **기기 미등록 시 발송 사전 차단 및 2가지 대안 안내 모달 표출**:
   - 연동된 기기나 상용 API 키가 없을 때는 발송을 즉시 중단하고, 1번 시트봇 에이전트(SheetBot Agent) 스마트폰 연동(무제한 무료)과 2번 상용 API 설정을 안내하는 모달 다이얼로그(`showSmsDeviceNoticeModal`)를 필수로 띄워야 합니다.
...
<!-- BEGIN:egdesk-db-watcher-rules -->
## 이지데스크 DB 왓처(DB Watcher) 사전 테스트 및 단계적 적용 원칙

1. **사전 검증(Pre-flight Testing) 필수 원칙**:
   - 이지데스크 서버가 제공하는 DB 왓처(DB Watcher / 실시간 테이블 변경 감지) 기능을 서비스 및 대시보드에 연동할 때는, **반드시 독립된 테스트 스크립트 또는 격리된 검증 환경에서 사전 동작 검증을 완료한 후 적용**해야 합니다.
   - 검증 항목:
     - DB 레코드 변경(INSERT/UPDATE) 시 이벤트 전달 프로토콜 규격 (WebSocket, SSE, 웹훅 등)
     - 지연 시간(Latency) 및 전달 보장성
     - 인증 헤더 및 권한 체계
2. **미검증 임의 스트림 급조 금지 (재발 방지)**:
