...
- **⚡ [필수] 이지데스크 DB 왓처(DB Watcher) 사전 테스트 및 단계적 적용 원칙**:
  - 이지데스크 서버가 제공하는 DB 왓처(DB Watcher / 변경 감지) 기능을 서비스에 연동할 때는, **반드시 독립 테스트 환경에서 사전 동작 검증(이벤트 수신 방식, 지연 시간, 프로토콜 규격)을 100% 완료한 후 프로덕션 코드에 적용**합니다.
  - 검증되지 않은 임의의 외부 스트림이나 가설 기반의 급조 코드를 도입하지 않으며, 테스트 결과와 프로토콜 명세를 명확히 확인한 후 단계적으로 통합합니다.

### 3) 향후 개발 로드맵
- [ ] **이지데스크 DB 왓처(DB Watcher) 독립 검증 및 무통장 입금 연동**: 이지데스크 서버의 DB 변경 감지 기능을 사전 테스트로 완벽히 검증한 후, 에이전트 M 대시보드 실시간 반영 엔진으로 공식 전환.
